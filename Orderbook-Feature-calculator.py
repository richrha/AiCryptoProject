...
def top_price_calculator(row, count):
    ...
    if (count)%level == 0: #top row of bid or ask
        if row.iloc[0, 2] == 0: #bid
            ...
            History.iloc[1, 2] = top_price[0]
        
        if row.iloc[0, 2] == 1: #ask
            top_price[1] = row.iloc[0,0]
            History.iloc[1,3] = top_price[1]
...
def bookv_calculator(count):

    if (count + 1)%(level*2) == 0 and count != 0: #at each point when a group ends
        if History.iloc[0,0] == 0:
            History.iloc[0] = History.iloc[1]
...
def main():
    ...
    History = pd.DataFrame({'bidQty':[0, 0], 'askQty':[0, 0], 'bidTop':[0, 0], 'askTop':[0, 0]}, index = ['0', '1'])
    ...
    for i in range(len(orderbook)):
        ...
        if (i + 1)%(level*2) == 0 and i != 0: 
            write_csv(features, headname+'-features.csv')
